...
if $AMD_GPU; then
    ...
    echo "* 5) Attach existing policies directly -> Filter policies: S3                                     *"
    echo "* 6) Tick 'AmazonS3ReadOnlyAccess'                                                                *"
    echo "* 7) Next: Tags -> Next: Review -> Create user                                                    *"
    echo "* 8) Copy and paste 'Access key ID' into the terminal -> Enter                                    *"
    echo "* 9) Copy and paste 'Secret access key' into the terminal (click 'Show') -> Enter                 *"
    echo "* 10) Default region name [None]: -> enter your region (f.e. eu-west-1)                           *"
    echo "* 11) Default output format [None]: -> leave empty and press Enter                                *"
    echo "*                                                                                                 *"
    echo "***************************************************************************************************"
    echo ""
    aws configure
